...
export const jargonDictionary: Record<string, JargonTerm> = {
  ...
  zsh: {
    ...
    analogy: "If bash is a reliable basic car that gets you from A to B, zsh is that same car with added features: a backup camera, lane assist, better GPS, and more comfortable seats. It still drives the same way; you don't need to relearn how to drive. But every little improvement adds up to a nicer experience, especially if you're driving (using the terminal) for hours each day.",
  ...
  "oh-my-zsh": {
    ...
    analogy: "Think of your terminal as a bare apartment. Oh My Zsh is like a furniture and decoration package that transforms it into a comfortable, functional home. Instead of blank walls, you get organized displays showing useful information. Instead of stumbling in the dark, you get smart lighting that highlights what matters. The plugins are like appliances: one gives you auto-suggestions while typing, another color-codes your commands, another provides shortcuts for common tasks. You can add or remove these as you like.",
  ...
  cli: {
    ...
    analogy: "A graphical interface is like driving a car with a steering wheel, pedals, and a touchscreen dashboard where you tap buttons for everything. A CLI is like typing exact instructions to a very literal chauffeur: 'drive forward 100 meters, turn left, stop.' Both get you where you're going, but the CLI approach is more precise, can be written down and repeated exactly, and works great for complex sequences of actions. The tradeoff is you need to learn the command vocabulary.",
  ...
  tmux: {
    ...
    analogy: "Imagine a TV that keeps playing even when you walk away. When you come back, you haven't missed anything; the show is exactly where it was. That's tmux. Or think of a workspace with multiple desks arranged side by side. You can see several projects at once, walk away for hours, and return to find everything undisturbed. The work continues even when you're not watching. This is in stark contrast to a regular terminal, which is more like a phone call: if you hang up (disconnect), the conversation ends.",
  ...
  git: {
    ...
    analogy: "Think of Git as 'undo' on steroids combined with a time machine. Every save point is remembered forever. You can name your saves ('Fixed the login bug') so you remember what each one accomplished. You can compare any two versions to see exactly what changed. You can create a 'branch' (like a parallel universe copy) to try experiments without affecting the main version, and merge your experiments back in if they work out. If an AI assistant makes changes you don't like, you can revert to before those changes existed. It's impossible to permanently lose work if it was ever committed to Git.",
    why: "Git is non-negotiable for any serious project. It prevents disasters ('I accidentally deleted everything') because deleted files can be recovered from history. It enables collaboration because everyone can work on their own copy and merge changes together. It provides accountability because every change is attributed to someone. And for AI-assisted development specifically, it provides a safety net: if an AI makes a mistake, you can see exactly what it changed and undo it. The installer sets up Git on your server because every project you create should be version-controlled.",
  ...
  lazygit: {
    ...
    long: "lazygit is a visual, keyboard-driven interface for Git that runs inside your terminal. Instead of typing Git commands from memory and trying to visualize what's happening, lazygit shows you everything at once in organized panels: files you've changed, changes ready to be saved, your history of saves, different versions you're working on. You navigate using arrow keys and perform actions with single-key shortcuts (press 'space' to mark a file for saving, 'c' to save with a message, 'p' to upload to the cloud). It makes the abstract concept of version control visual and interactive. Think of it as training wheels for Git: once you understand what's happening visually, the text commands start making sense too.",
  ...
  node: {
    ...
    long: "Node.js is what lets JavaScript (the programming language of the web) run on servers and computers outside of web browsers. Historically, JavaScript could only run inside web browsers and was used for making websites interactive. In 2009, Node.js changed this by creating a way to run JavaScript anywhere. This was revolutionary because web developers, who already knew JavaScript from building websites, could suddenly use the same language for server-side code, command-line tools, and more. The result is that JavaScript became one of the most versatile languages: the same skills work for websites, servers, mobile apps, and desktop applications. npm (Node Package Manager) provides access to over 2 million pre-built code libraries.",
